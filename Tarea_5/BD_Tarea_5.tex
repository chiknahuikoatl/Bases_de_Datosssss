...
\item{
    ...
    \item{¿Para qué sirve el concepto de dependencia en la normalización?\\
        Para eliminar o en su defecto reducir la redundancia en un base de
        datos.
    }
...
\begin{itemize}
    ...
    \begin{itemize}
    \item[a.] La única violación a la 3NF es D$\rightarrow$F.\\
        Podemos observar también que F se minimiza de la siguiente manera:\\
        F'=\{B $\rightarrow$ ED, D $\rightarrow$ F, AB $\rightarrow$ C\}\\
	\item[b.]\{A\}+ = \{ABCDE\} y \{E\}+ = \{EABCD\} son llaves, entonces la
        única violación a la 3NF es B $\rightarrow$ D.
    \end{itemize}
	\item Normaliza de acuerdo a la 3NF\\
    \begin{itemize}
    \item[a.]Superfluos por la izquierda: AB$\rightarrow$\\
    ¿A es superfluo por la izquierda? \{B\}+=\{BDEF\}, entonces A no es superfluo.\\
    ¿B es superfluo? \{A\}+=\{A\}, entonces B no es superfluo.\\
    Superfluos por la derecha: B $\rightarrow$ ED\\
    ¿E es superfluo? \{B\}+=\{ED\}, entonces E no lo es.\\
    ¿D es superfluo? \{B\}+=\{ED\}, entonces D no lo es.\\
    Por lo tanto, el esquema en 3NF es:
    S(B,E,D), T(D,F), U(A,B,C)
	\item[b.] Superfluos por la izquierda:  CD $\rightarrow$ E\\
